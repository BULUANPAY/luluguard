...
export async function POST(request: Request) {
  ...
  try {
    ...
    const targetDirectory = path.join(repositoryRoot, "uploaded-files", orderId);
    ...
    for (const { file, contents } of validatedFiles) {
      ...
      uploaded.push({
        ...
        storedName,
        size: file.size,
        path: path.posix.join("uploaded-files", orderId, storedName)
      });
    }

    return NextResponse.json({ orderId, uploaded }, { status: 201 });
  } catch (error) {
