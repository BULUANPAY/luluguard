...
export function JsonPreview({
      ...
      <div className="grid min-h-[420px] place-items-center rounded-2xl border border-dashed border-white/20 bg-white/5 p-8 text-center">
        <div>
          ...
          <p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-white/55">
            確認左側內容後點擊「產生 JSON 預覽」，資料會在這裡以 API payload
            格式呈現。
          </p>
        </div>
      </div>
  ...
  const displayReference = reference ?? getPayloadReference(payload);
  const copy = async () => {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/15 bg-[#102b25] shadow-2xl shadow-black/15">
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
        <div>
          <p className="font-mono text-xs font-bold text-[#d9f99d]">
            {fileName}
          </p>
          {displayReference ? (
...
function getPayloadReference(payload: unknown) {
  if (typeof payload !== "object" || payload === null) return undefined;
  if ("reference" in payload && typeof payload.reference === "string")
    return payload.reference;
  if ("document_id" in payload && typeof payload.document_id === "string")
