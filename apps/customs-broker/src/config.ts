...
export function readConfig(): BrokerConfig {
  ...
  if (
    !Number.isSafeInteger(loaded.port) ||
    loaded.port < 1 ||
    loaded.port > 65_535
  ) {
  ...
  if (
    !Number.isSafeInteger(loaded.quoteTtlSeconds) ||
    loaded.quoteTtlSeconds <= 0
  ) {
    throw new Error(
      "CUSTOMS_BROKER_QUOTE_TTL_SECONDS must be a positive integer",
    );
  }
  if (
    !Number.isSafeInteger(loaded.facilitatorTimeoutMs) ||
    loaded.facilitatorTimeoutMs <= 0
  ) {
